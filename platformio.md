...
All combinations of V1 and V2 ESP32 boards and display boards are supported by different targets in PlatformIO. The defines needed by the code to adapt for the different versions are included in the [platformio.ini](platformio.ini) config file.

One advantage of the V2 ESP32 board is you can buy a version with a connector for a larger WiFi antenna than the small built in antenna on V1 and other versions for the WiFi support.
...
The project used to use TFT_eSPI but the configuration for it was not simple and outside the normal tree. The only reason I was using it was for nicer fonts and those fonts are now supported directly by Adafruit graphics libraries so I have switched over to those. PlatformIO will download the correct libraries via the [platformio.ini](platformio.ini) configuration.
...
If you use LittleFS the first time the ESP32 is powered on after the Turning Feather code is installed it will format the onboard filesystem then fail to load files from it since it is empty. You can then transfer the files in [data](data/) with [the Arduino IDE uploader](https://randomnerdtutorials.com/esp32-vs-code-platformio-littlefs/).
